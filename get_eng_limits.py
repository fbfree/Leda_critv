...
if __name__ == '__main__':
    ...
    index = 3 # Calculate displacements at 3rd layer, under the rubber pads.
    ...
    speed_effects[:,1] = numpy.sqrt(numpy.max(displacement_arr[:,:,1+index*2]**2 + displacement_arr[:,:,2+index*2]**2,axis=1))

    # Find the maximum ground acceleration as a function of velocity
    extended_arr = numpy.concatenate((displacement_arr[:,:,(0,1+index*2,2+index*2)][:,-1,numpy.newaxis,:],displacement_arr[:,:,(0,1+index*2,2+index*2)],displacement_arr[:,:,(0,1+index*2,2+index*2)][:,1,numpy.newaxis,:]), axis=1)
    ...
    # Search through to find the lowest speed at which the peak ground acceleration exceeds 0.35 m/s^2 per UIC 776-2 specifications.  If no limit is found, report the maximum speed given in the data.
    ...
    for i,a in enumerate(speed_effects[:,2]):
        if a > 0.35:
            if i:
                ...
                max_acc = 0
            break
    print("Maximum velocity to maintain surface acceleration under 0.35 m/s2 is %f m/s \n" % max_acc)
